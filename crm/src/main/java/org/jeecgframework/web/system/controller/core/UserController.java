...
public class UserController extends BaseController
{
	...
	public AjaxJson del(TSUser user, HttpServletRequest req)
	{
		...
		if(!user.getStatus().equals(Globals.User_ADMIN))
		{
			...
			if(roleUser.size() > 0)
			{ // 删除用户时先删除用户和角色关系表 delRoleUser(user);

				// 删除用户时先删除用户和角色关系表
				systemService.executeSql("delete from t_s_role_user where userid=?", user.getId()); // 删除 用户-机构 数据
	...
	public AjaxJson trueDel(TSUser user, HttpServletRequest req)
	{
		 ...
		 * systemService.findByProperty(TSRoleUser.class, "TSUser.id",
		 * user.getId()); if (!user.getStatus().equals(Globals.User_ADMIN)) { if
		 * (roleUser.size()>0) { // 删除用户时先删除用户和角色关系表 delRoleUser(user);
		 * systemService.executeSql("delete from t_s_user_org where user_id=?",
		 * user.getId()); // 删除 用户-机构 数据 userService.delete(user); message =
		 * "用户：" + user.getUserName() + "删除成功"; systemService.addLog(message,
		 * Globals.Log_Type_DEL, Globals.Log_Leavel_INFO); } else {
		 * userService.delete(user); message = "用户：" + user.getUserName() +
		 * "删除成功"; } } else { message = "超级管理员不可删除"; }
		 */
	 ...
	 * roleUserList = systemService.findByProperty(TSRoleUser.class,
	 * "TSUser.id", user.getId()); if (roleUserList.size() >= 1) { for
	 * (TSRoleUser tRoleUser : roleUserList) { systemService.delete(tRoleUser);
	 * } } }
	 */
	/**
	 * 检查用户名
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user)
	{
		...
		else
		{
			...
			else
			{
				...
				String id = systemService.save(user).toString();
				user.setId(id);
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req, Model model)
	{

		List<TSDepart> tsList = new ArrayList<TSDepart>();
		List<Map<String, Object>> mapList = systemService.findForJdbc(
			"select id,departname,org_code from t_s_depart where org_type=2", null);
		for (Map<String, Object> map : mapList)
		{
			TSDepart domain = new TSDepart();
			domain.setId(map.get("id").toString());
			domain.setDepartname(map.get("departname").toString());
			domain.setOrgCode(map.get("org_code").toString());
			tsList.add(domain);
		}

		List<TSRole> roList = new ArrayList<TSRole>();
		List<Map<String, Object>> roMapList = systemService.findForJdbc("select id,rolecode,rolename from t_s_role ",
			null);
		for (Map<String, Object> map : roMapList)
		{
			TSRole domain = new TSRole();
			domain.setId(map.get("id").toString());
			domain.setRoleCode(map.get("rolecode").toString());
			domain.setRoleName(map.get("rolename").toString());
			roList.add(domain);
		}
	...
	public void idandname(HttpServletRequest req, TSUser user)
	{
		...
		req.setAttribute("roleId", roleId.subSequence(0, roleId.length() - 1));
	...
	public void getOrgInfos(HttpServletRequest req, TSUser user)
	{
		...
		req.setAttribute("orgIds", orgIds.subSequence(0, orgIds.length() - 1));
			...
			ModelMap modelMap)
	{
		...
		for (int i = 0; i < tsUsers.size(); i++)
		{
			...
			List<TSRole> roles = systemService
					.getSession()
					.createSQLQuery(
						"select * from t_s_role where id in (select roleid from t_s_role_user where userid=:userid)")
			...
			List<TSDepart> departs = systemService
					.getSession()
					.createSQLQuery(
						"select * from t_s_depart where id in (select org_id from t_s_user_org where user_id=:userid)")
		...
		modelMap.put(NormalExcelConstants.PARAMS, new ExportParams("用户表列表", "导出人:"
