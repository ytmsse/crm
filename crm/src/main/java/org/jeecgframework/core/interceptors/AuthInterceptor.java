	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception
	{
		...
		else
		{
			...
			if(client != null && currLoginUser != null)
			{
				...
				//				if((!hasMenuAuth(requestPath, clickFunctionId, currLoginUser))
				//						&& !currLoginUser.getUserName().equals("admin"))
				//				{
				//					response.sendRedirect(request.getSession().getServletContext().getContextPath()
				//							+ "/loginController.do?noAuth");
				//					return false;
				//				}
				//
				//				//解决rest风格下 权限失效问题
				...
				//				}
				//
				//				List<TSFunction> functions = systemService.findByProperty(TSFunction.class, "functionUrl",
				//					realRequestPath);
				//				if(functions.size() > 0)
				//				{
				...
				//					Set<String> operationCodes = systemService.getOperationCodesByUserIdAndFunctionId(
				//						currLoginUser.getId(), functionId);
				...
				//							+ "' AND fun.operation is not null  AND fun.roleid=role.roleid AND role.userid='"
				//							+ currLoginUser.getId() + "' ";
				//					List<String> hasOperList = this.systemService.findListbySql(hasOperSql);
				//					for (String operationIds : hasOperList)
				//					{
				...
				//									&& (operation.getOperationcode().startsWith("#") || operation.getOperationcode()
				//											.startsWith(".")))
				//							{
	...
	private boolean hasMenuAuth(String requestPath, String clickFunctionId, TSUser currLoginUser)
	{
		...
		if(authSize <= 0)
		{
			...
			String functionOfOrgSql = "SELECT count(*) from t_s_function f, t_s_role_function rf, t_s_role_org ro  "
					+ "WHERE f.ID=rf.functionid AND rf.roleid=ro.role_id " + "AND ro.org_id='" + orgId
					+ "' AND f.functionurl = '" + requestPath + "'";
